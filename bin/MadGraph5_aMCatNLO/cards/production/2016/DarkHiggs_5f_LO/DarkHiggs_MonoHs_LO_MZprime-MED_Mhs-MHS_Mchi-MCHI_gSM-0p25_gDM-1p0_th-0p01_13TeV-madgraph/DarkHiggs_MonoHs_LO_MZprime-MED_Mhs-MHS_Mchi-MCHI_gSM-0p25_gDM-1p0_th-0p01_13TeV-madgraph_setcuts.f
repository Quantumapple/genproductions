...
c-charged-leptons
c         if (abs(idup(i,1,iproc)).eq.11)  is_a_l(i)=.true. ! e+  e-
c         if (abs(idup(i,1,iproc)).eq.13)  is_a_l(i)=.true. ! mu+ mu-
c         if (abs(idup(i,1,iproc)).eq.15)  is_a_l(i)=.true. ! ta+ ta-
         if (abs(idup(i,1,iproc)).eq.52) is_a_l(i)=.true. ! chi chi~ <\p> 
c-b-quarks
...
c            ptll_min(j,i)=0.0d0**2
c            ptll_max(j,i)=-1
c            if(((is_a_l(i).and.is_a_l(j)).and.
c     &           (abs(idup(i,1,iproc)).eq.abs(idup(j,1,iproc))).and.
c     &           (idup(i,1,iproc)*idup(j,1,iproc).lt.0)) ! Leptons from same flavor but different charge
c     &           .or.(is_a_nu(i).and.is_a_l(j))
c     &           .or.(is_a_l(i).and.is_a_nu(j)) !a lepton and a neutrino
c     &           .or.(is_a_nu(i).and.is_a_nu(j))) then ! two neutrinos 
c               ptll_min(j,i)=ptllmin*dabs(ptllmin)
c               ptll_max(j,i)=ptllmax*dabs(ptllmax)
c            endif
c         enddo
c      enddo
...
c CHANGE
      do i=nincoming+1,nexternal-1
         do j=i+1,nexternal
            ...
            ptll_max(j,i)=-1
            if(((is_a_l(i).and.is_a_l(j)).and.(abs(idup(i,1,iproc)).eq.abs(idup(j,1,iproc))))) then
